...
class LogitsProcessor(nn.Module):
    ...
    def compute_logprobs_for_multi_item_scoring(
        ...
        lm_head: VocabParallelEmbedding,
        logits_metadata: Union[LogitsMetadata, ForwardBatch],
        multi_item_delimiter_indices: List[Optional[torch.Tensor]],
    ):
        """
        ...
        """
        # Compute positions just before each delimiter (GPU ops, no sync).
        # Tensors are on CPU; .to(device) moves them for the cat/index ops.
        ...
        if logits_metadata.extend_seq_lens_cpu is not None:
            ...
            indices_tensor = multi_item_delimiter_indices[0]
            all_tensors.append(indices_tensor.to(device, non_blocking=True) - 1)
        multi_item_indices = torch.cat(all_tensors)
